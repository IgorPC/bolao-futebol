function submitForm(form)
{
    form.addEventListener('submit', function(event){
        ...
        let result = confirm("Tem certeza que deseja finalizar concluir o bolão?");
        if(result){
...
function updateProgress(action, input)
{
    ...
    if(counter[splitted[0]] == "" || counter[splitted[0]] == undefined){
        counter[splitted[0]] = 1
    }else{
        if(action == '-1'){
            counter[splitted[0]] -= 1
        }
    ...
    if(action == '-1'){
        if(counter[splitted[0]] == 0){
            list[actual-1] = false
            let total = checkInputs.length/3;
            if(count <= 0 ){
                count = 1
            }
            let now = count -= 1;
